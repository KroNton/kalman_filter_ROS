...
"""
  ...
  @brief   Auxiliary ROS node that subcribes to movement commands
           and simulates a noisy odometry and a noisy light sensor.
           Use in conjunction with the Discrete Bayes Filter Algorithm ROS Node
  @date    July 03, 2020
...
def movement_cmd_callback(data):
    ...
    movement_noise = np.random.choice(a=[-1,0,1], p=odometry_noise_profile)
    ...
    sensor_reading_noisy = np.random.choice(a=[abs((corridor_map[ground_truth_position])-1),corridor_map[ground_truth_position]], p=[1-light_sensor_accuracy_rate, light_sensor_accuracy_rate])
    print("\n")
